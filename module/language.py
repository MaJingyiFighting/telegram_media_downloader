...
translations = {
    ...
    "Download speed": ["下载速度", "Скорость скачивания", "Швидкість скачування"],
    "is below minimum": ["低于最低要求", "ниже минимума", "нижче мінімуму"],
    "monitoring started": ["开始监控", "мониторинг начат", "моніторинг почато"],
    "Download speed has been below": ["下载速度持续低于", "Скорость скачивания была ниже", "Швидкість скачування була нижче"],
    "for": ["持续", "в течение", "протягом"],
    "restarting program": ["重启程序", "перезапуск программы", "перезапуск програми"],
    "is back to normal": ["恢复正常", "вернулась в норму", "повернулася до норми"],
    "No download activity for": ["无下载活动已持续", "Нет активности загрузки в течение", "Немає активності завантаження протягом"],
    "possible connection issue": ["可能存在连接问题", "возможная проблема с подключением", "можлива проблема з підключенням"],
    "Connection issue detected": ["检测到连接问题", "Обнаружена проблема с подключением", "Виявлено проблему з підключенням"],
    "since last activity": ["自上次活动以来", "с момента последней активности", "з моменту останньої активності"],
}
